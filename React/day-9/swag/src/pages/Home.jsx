...
const Home = () => {
  let { setCartItems, cartItems } = useContext(MyStore);
  ...
  let addToCart = (id) => {
    ...
    // let isExist = cartItems.find((val) => val.id === product.id);
    // if (isExist) {
    //   isExist.quantity++;
  ...
  let handleDecrement = (id) => {
    ...
    setCartItems((prev) => {
      ...
      if (isExist.quantity <= 1) {
        return prev.filter((val) => val.id !== product.id);
      }

      return prev.map(
        (val) => val.id === product.id && { ...val, quantity: val.quantity - 1 }
      );
    });
  };
